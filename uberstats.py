...
class uberstats(minqlx.Plugin):
  ...
  def __init__(self):
    ...
    self.pewpewpew_names = []
    self.pewpewpew = 0

    self.highest_score_names = []
    self.highest_score = 0

    self.best_td_names = []
    self.best_td = 0

    self.most_thaws_names = []
    self.most_thaws = 0
    ...
    if stats['TYPE'] == "PLAYER_STATS":
      ...
      if stats['DATA']['QUIT'] == 0 and stats['DATA']['WARMUP'] == 0:
        if stats['DATA']['PLAY_TIME'] >= 120 and stats['DATA']['STEAM_ID'] != "0": #ignore bots:
          ...
          player_thaws = stats['DATA']['MEDALS'].get('ASSISTS', 0)

          player_score = stats['DATA']['SCORE']
          player_deaths = stats['DATA']['DEATHS']

          # Player Thaw / Death ratio
          if player_deaths != 0: #we don't want to divide by 0!
            player_td = player_thaws / player_deaths
          else:
            player_td = player_thaws
          
          # player_quads = stats['DATA']['PICKUPS'].get('QUAD', 0)          
          ...
          if not self.most_thaws_names:
            self.most_thaws_names = [player_name]
            self.most_thaws = player_thaws
          elif player_thaws > self.most_thaws:
            self.most_thaws_names = [player_name]
            self.most_thaws = player_thaws
          elif player_thaws == self.most_thaws:
            self.most_thaws_names.append(player_name)

          if not self.highest_score_names:
              self.highest_score_names = [player_name]
              self.highest_score = player_score
          elif player_score > self.highest_score:
              self.highest_score_names = [player_name]
              self.highest_score = player_score
          elif player_score == self.highest_score:
              self.highest_score_names.append(player_name)

          if not self.best_td_names:
              self.best_td_names = [player_name]
              self.best_td = player_td
          elif player_td > self.best_td:
              self.best_td_names = [player_name]
              self.best_td = player_td
          elif player_td == self.best_td:
              self.best_td_names.append(player_name)

          if not self.pewpewpew_names:
              self.pewpewpew_names = [player_name]
              self.pewpewpew = player_rail_hits
          elif player_rail_hits > self.pewpewpew:
              self.pewpewpew_names = [player_name]
              self.pewpewpew = player_rail_hits
          elif player_rail_hits == self.pewpewpew:
              self.pewpewpew_names.append(player_name)
  ...
  def handle_game_end(self, data):
    if not data["ABORTED"] and self.best_kpm:
      ...
      if self.pewpewpew > 0:
        stats_output = "^5PEWPEWPEW: "
        record_response = ""
        for i, player_name in enumerate(self.pewpewpew_names):
            record_response = self.check_record("pewpewpew", float(self.pewpewpew), player_name)
            stats_output += "^7" + player_name
            if len(self.pewpewpew_names) > 1 and len(self.pewpewpew_names) - 1 != i:
                stats_output += ", "
        stats_output += "^2 - {} rail hits".format(self.pewpewpew)
        self.msg(record_response + stats_output)
      ...
      if self.game.type not in ["Duel", "Instagib"]:
        ...
        for i, player_name in enumerate(self.most_dmg_per_kill_names):
          ...
          if len(self.most_dmg_per_kill_names) > 1 and len(self.most_dmg_per_kill_names) - 1 != i:
            stats_output += ", "
        stats_output += "^2 - {:0.2f} dmg per frag".format(self.most_dmg_per_kill)
        self.msg(record_response + stats_output)
      ...
      if self.best_td > 0:
        stats_output = "^5THAW TACTICIAN: "
        record_response = ""
        for i, player_name in enumerate(self.best_td_names):
            record_response = self.check_record("best_td_ratio", float(self.best_td), player_name)
            stats_output += "^7" + player_name
            if len(self.best_td_names) > 1 and len(self.best_td_names) - 1 != i:
                stats_output += ", "
            stats_output += "^2 - {:.2f} T/D ratio".format(self.best_td)
        self.msg(record_response + stats_output)

      if self.most_thaws > 0:
        stats_output = "^5LIFESAVER: "
        record_response = ""
        for i, player_name in enumerate(self.most_thaws_names):
            record_response = self.check_record("most_thaws", float(self.most_thaws), player_name)
            stats_output += "^7" + player_name
            if len(self.most_thaws_names) > 1 and len(self.most_thaws_names) - 1 != i:
                stats_output += ", "
        stats_output += "^2 - {} thaws".format(self.most_thaws)
        self.msg(record_response + stats_output)

      if self.highest_score > 0:
        stats_output = "^1HIGHEST SCORE: "
        record_response = ""
        for i, player_name in enumerate(self.highest_score_names):
            record_response = self.check_record("highest_score", float(self.highest_score), player_name)
            stats_output += "^7" + player_name
            if len(self.highest_score_names) > 1 and len(self.highest_score_names) - 1 != i:
                stats_output += ", "
        stats_output += "^2 - {} pts".format(self.highest_score)
        self.msg(record_response + stats_output)
